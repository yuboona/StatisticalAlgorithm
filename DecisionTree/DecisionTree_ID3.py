...
# 信息熵是信息论中对于某一信息所含的信息量（价值、不确定性、知道这一消息后会产生巨大的影响）的一种描述  
# 公式如下：
# $H(信息熵) = -\sum_{x\in{X}} P(x) log_2(P(x))$
#
# - $log_2(P(x))$在数值上，为负的大于1的数字
# - $P(x)$指示出了权重
...
def plot_tree(my_tree, parent_pt, node_txt):
    ...
    cntr_pt = (plot_tree.x_off +
               (1.0 + float(num_leafs)) / 2.0 / plot_tree.total_w,
               plot_tree.y_off)
    ...
    for key in second_dict.keys():
        if type(second_dict[key]).__name__ == 'dict':
            ...
            plot_node(second_dict[key], (plot_tree.x_off, plot_tree.y_off),
                      cntr_pt, leaf_node)
            plot_mid_text((plot_tree.x_off, plot_tree.y_off), cntr_pt,
                          str(key))
    plot_tree.y_off = plot_tree.y_off + 1.0 / plot_tree.total_d
...
def create_data_set():
    ...
    :return:
    """
    data_set = [[1, 1, 'yes'], [1, 1, 'yes'], [1, 0, 'no'], [0, 1, 'no'],
                [0, 1, 'no']]
...
def choose_best_feature_to_split(data_set):
    ...
    for feature_idx in range(num_feature):
        feature_val_list = [number[feature_idx]
                            for number in data_set]  # 得到某个特征下所有值（某列）
...
def create_tree(data_set, labels):
    ...
    """
    class_list = [sample[-1]
                  for sample in data_set]  # ['yes', 'yes', 'no', 'no', 'no']
    # 类别相同，停止划分
    ...
    for feature_value in unique_feature_values:
        ...
        # 构建数据的子集合，并进行递归
        sub_data_set = split_data_set(data_set, best_feature_idx,
                                      feature_value)  # 待划分的子数据集
        my_tree[best_feat_label][feature_value] = create_tree(
            sub_data_set, sub_labels)
    return my_tree
